...
def test_tensor(device_name="cpu"):
    host_tensor = torch.arange(60, dtype=torch_dtype("i64")).reshape(3, 4, 5)
    torch_tensor = host_tensor.to(torch_device(device_name))
    ...
    # Test load
    print("===Test load===")
    llaisys_tensor.load(host_tensor.data_ptr())
