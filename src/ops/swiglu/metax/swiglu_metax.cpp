...
void launchSwiGLU(
    ...
    swigluKernel<<<elementwiseBlocks(numel, threads), threads, 0, stream>>>(
        ...
        numel);
    device::metax::checkKernelLaunch("SwiGLU kernel launch");
}
...
void swiglu(
    ...
    case LLAISYS_DTYPE_F32:
        return launchSwiGLU<float>(out, gate, up, numel, macaStream(stream));
    case LLAISYS_DTYPE_F16:
        return launchSwiGLU<__half>(out, gate, up, numel, macaStream(stream));
    case LLAISYS_DTYPE_BF16:
        return launchSwiGLU<__nv_bfloat16>(out, gate, up, numel, macaStream(stream));
    default:
