...
void launchBias(
    ...
    addBiasKernel<<<elementwiseBlocks(numel, threads), threads, 0, stream>>>(
        ...
        numel,
        out_features);
    device::metax::checkKernelLaunch("linear bias kernel launch");
}
...
void linear(
    ...
    if (batch > INT_MAX || in_features > INT_MAX || out_features > INT_MAX) {
        throw std::overflow_error("linear dimensions exceed mcBLAS integer limits");
    }
    ...
    cublasHandle_t handle = device::metax::resource(device_id).mcblas(cuda_stream);
    const cublasGemmAlgo_t algorithm =
        dtype == LLAISYS_DTYPE_F32
            ? CUBLAS_GEMM_DEFAULT
            : CUBLAS_GEMM_DEFAULT_TENSOR_OP;

    device::metax::checkMcblas(
        cublasGemmEx(
            handle,
            CUBLAS_OP_T,
            CUBLAS_OP_N,
