...
void launchArgmax(
    ...
    if (numel <= kSingleBlockMaxElements) {
        argmaxSingleBlockKernel<<<1, kThreads, 0, stream>>>(
            ...
            numel);
        device::metax::checkKernelLaunch("argmax kernel launch");
        return;
    }
    ...
    const auto workspace =
        device::metax::resource(device_id).argmaxWorkspace(blocks);
    argmaxStageOneKernel<<<blocks, kThreads, 0, stream>>>(
        workspace.values,
        workspace.indices,
        reinterpret_cast<const T *>(vals),
        numel);
    device::metax::checkKernelLaunch("argmax stage-one kernel launch");
    ...
    argmaxStageTwoKernel<<<1, kThreads, 0, stream>>>(
        ...
        workspace.indices,
        blocks);
    device::metax::checkKernelLaunch("argmax stage-two kernel launch");
}
...
void argmax(
    ...
    case LLAISYS_DTYPE_F32:
        return launchArgmax<float>(
            max_idx, max_val, vals, numel, device_id, macaStream(stream));
