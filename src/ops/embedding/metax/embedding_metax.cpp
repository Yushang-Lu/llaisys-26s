...
void launchEmbedding(
    ...
    embeddingKernel<<<elementwiseBlocks(numel, threads), threads, 0, stream>>>(
        ...
        num_embeddings,
        hidden_size);
    device::metax::checkKernelLaunch("embedding kernel launch");
}
...
void embedding(
    ...
    case LLAISYS_DTYPE_F32:
        return launchEmbedding<float>(out, index, weight, num_indices, num_embeddings, hidden_size, macaStream(stream));
    case LLAISYS_DTYPE_F16:
        return launchEmbedding<__half>(out, index, weight, num_indices, num_embeddings, hidden_size, macaStream(stream));
    case LLAISYS_DTYPE_BF16:
        return launchEmbedding<__nv_bfloat16>(out, index, weight, num_indices, num_embeddings, hidden_size, macaStream(stream));
    default:
