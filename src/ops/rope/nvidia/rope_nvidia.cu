...
void launchRope(
    ...
    ropeKernel<<<elementwiseBlocks(total_pairs, threads), threads, 0, stream>>>(
        ...
        head_dim,
        theta);
    device::nvidia::checkKernelLaunch("RoPE kernel launch");
}
...
void rope(
    ...
    case LLAISYS_DTYPE_F32:
        return launchRope<float>(out, in, pos_ids, seq_len, num_heads, head_dim, theta, cudaStream(stream));
    case LLAISYS_DTYPE_F16:
        return launchRope<__half>(out, in, pos_ids, seq_len, num_heads, head_dim, theta, cudaStream(stream));
    case LLAISYS_DTYPE_BF16:
        return launchRope<__nv_bfloat16>(out, in, pos_ids, seq_len, num_heads, head_dim, theta, cudaStream(stream));
    default:
