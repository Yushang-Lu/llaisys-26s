...
void launchRmsNorm(
    ...
    rmsNormKernel<<<static_cast<unsigned int>(rows), 256, 0, stream>>>(
        ...
        hidden_size,
        eps);
    device::nvidia::checkKernelLaunch("RMSNorm kernel launch");
}
...
void rms_norm(
    ...
    case LLAISYS_DTYPE_F32:
        return launchRmsNorm<float>(out, in, weight, rows, hidden_size, eps, cudaStream(stream));
    case LLAISYS_DTYPE_F16:
        return launchRmsNorm<__half>(out, in, weight, rows, hidden_size, eps, cudaStream(stream));
    case LLAISYS_DTYPE_BF16:
        return launchRmsNorm<__nv_bfloat16>(out, in, weight, rows, hidden_size, eps, cudaStream(stream));
    default:
