...
#include "../../../device/metax/metax_resource.hpp"
...
void launchAttention(
    ...
    if (rows > std::numeric_limits<unsigned int>::max() ||
        ...
        throw std::overflow_error("self-attention dimensions exceed MetaX grid limits");
    }
    ...
    attentionScoreKernel<<<static_cast<unsigned int>(score_count), score_threads, 0, stream>>>(
        ...
        head_dim,
        scale);
    device::metax::checkKernelLaunch("self-attention score kernel launch");

    attentionSoftmaxKernel<<<static_cast<unsigned int>(rows), softmax_threads, 0, stream>>>(
        scores, q_len, kv_len, num_heads);
    device::metax::checkKernelLaunch("self-attention softmax kernel launch");
    ...
    attentionValueKernel<<<static_cast<unsigned int>(rows), value_threads, 0, stream>>>(
        ...
        num_kv_heads,
        value_dim);
    device::metax::checkKernelLaunch("self-attention value kernel launch");
}
...
void self_attention(
    ...
    float *scores = device::metax::resource(device_id).attentionScores(score_count);
    const cudaStream_t cuda_stream = macaStream(stream);
